...
const App: React.FC = () => {
    ...
    const findNextTask = (
        ...
        const isVlmFolder = folderTasks.length > 0 && folderTasks.some(t => t.sourceType === 'vlm-review');
        folderTasks.sort((a, b) => isVlmFolder ? a.id.localeCompare(b.id) : a.name.localeCompare(b.name, undefined, { numeric: true }));
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const overview = await Storage.getProjectOverview();
                const projectId = overview.projectMap[currentTask.folder]?.projectId;
                if (!projectId || cancelled) return;
                const detail = await Storage.getProjectDetail(projectId);
                if (!detail?.project?.classificationClasses || cancelled) {
                    setClassificationClassesForPanel([]);
                    return;
                }
                const classes: YoloClass[] = (detail.project.classificationClasses as Array<{ id: number; name: string }>).map((c, i) => ({
                    id: c.id,
                    name: c.name,
    ...
    }, [currentTask?.id, currentTask?.folder, currentTask?.sourceType]);

    const handleClassificationSave = useCallback(async (classId: number | null) => {
        if (!currentTask || !user) return;
        const sourceData = classId != null ? JSON.stringify({ classId }) : undefined;
        await Storage.updateTask(currentTask.id, { sourceData, isModified: true }, user.username, currentUserRole);
        const updated = Storage.getTasks().find(t => t.id === currentTask.id) || currentTask;
        setCurrentTask({ ...updated, sourceData });
        setTasks(Storage.getTasks());
