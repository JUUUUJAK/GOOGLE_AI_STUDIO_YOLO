...
const buildDatasetsUrl = (limit: number, offset: number): string => {
...
export const initStorage = async () => {
  try {
    ...
    const res = await fetch(buildDatasetsUrl(INITIAL_TASK_FETCH_LIMIT, 0));
    if (!res.ok) throw new Error('Failed to fetch datasets');
    const files = await res.json();

    // We intentionally do NOT restore task states from local cache.
    // Task source-of-truth must be server only.
    ...
    cachedTasks = files.map((f: any) => normalizeServerTask(f)) as Task[];
    await persistCachedTasks();
    refreshCommittedTaskSignatures();

    // On app bootstrap, avoid heavy full-log pull.
    // Push/pull logs can be triggered on-demand (e.g., History view).
...
export const syncAllTaskPages = async (limit: number = INITIAL_TASK_FETCH_LIMIT) => {
  try {
    ...
    while (true) {
      const res = await fetch(buildDatasetsUrl(limit, offset));
      if (!res.ok) throw new Error('Failed to fetch datasets');
...
export const importVlmJsonData = async (payload: {
  ...
  try {
    ...
    if (!res.ok) throw new Error('Failed to import VLM json data');
    return await res.json();
  } catch (e) {
