      ...
      Object.values(projects).forEach((project: any) => {
        ...
        try {
          const row = db.prepare(`
            SELECT 
              COUNT(CASE WHEN assignedWorker IS NOT NULL AND TRIM(LOWER(assignedWorker)) NOT IN ('', 'unassigned', 'admin') THEN 1 END) as taskCount, 
          ...
          `).get(project.vlmSourceFile) as { taskCount: number; completedCount: number };
          const current = statsByProject.get(project.id) || { allocated: 0, completed: 0, folderCount: 0 };
          statsByProject.set(project.id, {
            allocated: current.allocated + Number(row?.taskCount || 0),
            completed: current.completed + Number(row?.completedCount || 0),
            folderCount: current.folderCount + (Number(row?.taskCount || 0) > 0 ? 1 : 0)
          });
        } catch (e) { }
      });
